---
title: "Next.js App Router Caching — Cheat Sheet"
...
- **Data Cache (server, persistent)**  
  ...
  - `force-cache` → uses it.  
  - `no-store` → skips it.  
  - `next.revalidate` → ISR-style time-based revalidation.  
...
- In **browser** → `cache` controls the HTTP cache.  
- In **Next.js server** → `cache` controls the **Data Cache** behavior.
