...
- First paint = light.  
- After hydration, `dark` class is applied → second paint.  
- Visible **flash of light → dark**.
...
- `getInitColorSchemeScript()` → ensures correct theme before first paint.  
- `CssVarsProvider` → enables light/dark CSS variable system.  
